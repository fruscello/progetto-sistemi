...
void interruptHandler()
{
    ...
    else
    {
		...
		tprint("\nINT PRINTER\n");
		//tprint("\nHELLOOOOOOOO\n");
                //p = headBlocked(&normalDevices[i-INT_LOWEST][j]);   
		//p=headBlocked(&device_mutex[j][PRINTER]);			
		/* INT_LOWEST is an offset of 3,
		...
		//V(&device_mutex[j][PRINTER],(state_t*)INT_OLDAREA);
                //deviceRegister->dtp.command = DEV_C_ACK; /* acknowledging the interrupt */
                break;
	case INT_TERMINAL:
                tprint("in INT_TERMINAL\n");
		
		unsigned int* reg;
		state_t app;
		STST(&app);
		
		getDeviceRegister(INT_TERMINAL, 1,&reg);
		app.a1=reg[2];	
		LDST(&app);	
		while(1) tprint("IN WHILE\n");
/*
		int i=0;
		for(i=0;i<term_addr[1].length;i++)
			tprint("MAMMATA\n");
		still_soft_blocked=1;
		WTnext(1);
		term_addr[1].index++;
